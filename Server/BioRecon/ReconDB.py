...
def Query(user, room):
	...
	statement = "SELECT CASE WHEN EXISTS ( SELECT * FROM users JOIN rol_usr ON users.id=rol_usr.usr_id JOIN rol_room ON rol_usr.rol_id = rol_room.rol_id WHERE users.uid ='"+user+"' AND rol_room.rom_id ="+room+") THEN (SELECT MAX(rol.auth_level) FROM rol JOIN rol_usr ON rol.id=rol_usr.rol_id JOIN users ON rol_usr.usr_id = users.id WHERE users.uid ='"+user+"') ELSE 'FALSE' END"
	#print(statement)
	...
	if "FALSE" in str(result[0]):
		...
		statement= "SELECT rol_id FROM rol_room WHERE rom_id = '"+room+"'"
		mycursor.execute(statement)
		result = (mycursor.fetchall())
		if result == "1": 
			return True
